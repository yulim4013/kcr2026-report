...
const HEADERS     = [
  ...
  "영어능력", "기타언어",
  "희망업무", "5/13참여", "5/14참여", "5/15참여", "5/16참여",
  "상태", "상태변경일시", "메모",
  "최종배정파트",
  "출근_5/13", "퇴근_5/13",
  "출근_5/14", "퇴근_5/14",
...
function onStatusChange(e) {
  if (!e || !e.source) {
    Logger.log("onStatusChange는 onEdit 트리거를 통해서만 실행됩니다. setRecruitTriggers()로 트리거를 등록하세요.");
    return;
  }
...
function addMissingColumns() {
  ...
  // 1. "4/13참여" → "5/13참여" 처리
  const old413Idx = currentHeaders.indexOf("4/13참여");
  const cur513Idx = currentHeaders.indexOf("5/13참여");

  if (old413Idx >= 0 && cur513Idx < 0) {
    // 케이스 A: "4/13참여"만 있음 → 헤더 이름만 변경
    sheet.getRange(1, old413Idx + 1).setValue("5/13참여");
    styleHeader(old413Idx + 1);
    currentHeaders[old413Idx] = "5/13참여";
    Logger.log("이름 변경: 4/13참여 → 5/13참여");

  } else if (old413Idx >= 0 && cur513Idx >= 0) {
    // 케이스 B: 둘 다 있음(현재 오류 상태) → "4/13참여" 데이터를 "5/13참여"로 병합 후 삭제
    const lastRow = sheet.getLastRow();
    if (lastRow > 1) {
      const srcData = sheet.getRange(2, old413Idx + 1, lastRow - 1, 1).getValues();
      const dstData = sheet.getRange(2, cur513Idx + 1, lastRow - 1, 1).getValues();
      const merged  = srcData.map((src, i) => [dstData[i][0] || src[0]]);
      sheet.getRange(2, cur513Idx + 1, lastRow - 1, 1).setValues(merged);
    }
    sheet.deleteColumn(old413Idx + 1);
    currentHeaders.splice(old413Idx, 1);
    Logger.log("4/13참여 열 삭제 완료 (데이터 5/13참여로 이동)");
    ...
    if (rolesIdx >= 0) {
      ...
      const insertCol = rolesIdx + 2;
      sheet.getRange(1, insertCol).setValue("5/13참여");
      styleHeader(insertCol);
      currentHeaders.splice(rolesIdx + 1, 0, "5/13참여");
      Logger.log("추가: 5/13참여");
    }
  }
